...
const HubsPage = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8080/rental/getHubRentals"
        );
  ...
  return (
    <Page short={isShort}>
      ...
      <AlbyFilter
        selected={selectedFilter === "Changers Alby"}
        onClick={() => handleFilter("Changers Alby")}
      >
        Alby
      </AlbyFilter>
      <StureFilter
        selected={selectedFilter === "Changers Sture"}
        onClick={() => handleFilter("Changers Sture")}
      >
        Sture
      </StureFilter>
      <KistaFilter
        selected={selectedFilter === "Changers Kista"}
        onClick={() => handleFilter("Changers Kista")}
      >
        Kista
      </KistaFilter>
      <Container>
        ...
        {filteredEquipment.map((item) => (
          <Card key={item.id}>
            ...
            <Name>{item.name}</Name>
            <LocationIcon />
            <LocationTxt>{item.location}</LocationTxt>
